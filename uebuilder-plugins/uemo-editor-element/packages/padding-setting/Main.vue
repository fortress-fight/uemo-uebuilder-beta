...
import type { UeElPaddingSettingBaseProps } from "./index";

defineOptions({ name: "UeElPaddingSetting" });
const prop = withDefaults(defineProps<UeElPaddingSettingBaseProps>(), {
    disable: false,
    type: "xy",
    defaultValue: "0",
...
const onOffParam = computed<UE_EL_COMPONENT.UeElControlGroupProps>(() => ({
    ...
    onOffParam: {
        ...
        reverseStyle: true,
        value: isMix.value,
        icon: "icon-shangxiazuoyoujianju",
    },
}));
...
const t = computed({
    ...
    set(value) {
        changePadding([value, padding.value[1], padding.value[2], padding.value[3]]);
    },
});

const r = computed({
    get() {
        return padding.value[1];
    },
    set(value) {
        changePadding([padding.value[0], value, padding.value[2], padding.value[3]]);
    },
});
...
const b = computed({
    ...
    set(value) {
        changePadding([padding.value[0], padding.value[1], value, padding.value[3]]);
    },
});

const l = computed({
    get() {
        return padding.value[3];
    },
    set(value) {
        changePadding([padding.value[0], padding.value[1], padding.value[2], value]);
    },
});
...
const tb = computed({
    ...
    set(value) {
        changePadding([value, padding.value[1], value, padding.value[3]]);
    },
});
...
const lr = computed({
    ...
    set(value) {
        changePadding([padding.value[0], value, padding.value[2], value]);
    },
});
...
function toggleMixType(value: any) {
    isMix.value = value;
    changePadding([t.value, r.value, t.value, r.value]);
}

...
import { isSvgViewNode } from "../utils/helper";
import { getEditorPanelExtensionStorage } from "../../extension-editor-panel/utils/helper";

defineOptions({ name: "UeTiptapSvgViewMenu", inheritAttrs: false });
...
const shouldShow: UE_TIPTAP_COMPONENT.UeTiptapFloatingMenuProps["shouldShow"] = ({ editor, view, state }) => {
    ...
    const isSvgView = isSvgViewNode(selection);
    const isEditing = getEditorPanelExtensionStorage(editor).lastEditorPanelType === "svgView";

    return !isEditing && isSvgView && view.hasFocus();
};
</script>
